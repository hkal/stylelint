...
const messages = ruleMessages(ruleName, {
	rejected: (selector, priorSelector, priorLine) => `Expected selector "${selector}" to come before selector "${priorSelector} (L${priorLine})"`,
});
...
function rule(on, options) {
	return (root, result) => {
		...
		function checkSelector(selectorNode, rule, sourceIndex, comparisonContext) {
			...
			const entry = { selector, specificity: selectorSpecificity, line: rule.source.start.line };
			...
			priorComparableSelectors.forEach((priorEntry) => {
				if (specificity.compare(selectorSpecificity, priorEntry.specificity) === -1) {
					report({
						...
						result,
						node: rule,
						message: messages.rejected(selector, priorEntry.selector, priorEntry.line),
						index: sourceIndex,
					});
				}
			});
